...
export const useJobSearch = (title: string, location: string = 'remote', limit: number = 30) => {
  return useQuery({
    queryKey: ['jobs', title, location, limit],
    queryFn: () => jobsApi.searchJobs(title, location, limit),
    enabled: !!title
  });
};
